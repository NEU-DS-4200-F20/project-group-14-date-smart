...
((() => {
  ...
  //from https://bl.ocks.org/cagrimmett/07f8c8daea00946b9e704e3efcbd5739
  //creates the grid 
  function gridData() {
      var data = new Array();
    ...
    // iterate for rows	
    //replace number rows with number participants
    console.log(totalParticipants);
      for (var row = 0; row < 500; row++) {
          data.push( new Array() );
          
      // iterate for cells/columns inside rows
      //replace number of columns with number participants 
  ...
  let svg3 = d3.select('#vis-svg-3')
    ...
    /*
      ...
      var yAxis = d3.axisRight(yScale3)
       .ticks(100)
       .tickSize(height)
       .tickPadding(8 - width);
  ...
  /*
    ...
    .call(xAxis);
     ...
     svg3.append("text")
          ...
          .style("font-size", "16px") 
          .style("text-decoration", "underline")  
          .text("Date SMART Temporal Time Sequence");
